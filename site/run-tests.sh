...
# --pdb: will enter the pdb debug mode when tests fails automatically
# no:warnings: Hide warnings in console
# -s: Show print statments

# python -m pytest -s -p no:warnings --pdb ${pytest_args[@]+"${pytest_args[@]}"}
python -m pytest -p no:warnings ${pytest_args[@]+"${pytest_args[@]}"}
tests_exit_code=$?
